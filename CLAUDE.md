...
Famble is a Slay the Spire-style roguelike deckbuilder that generates unique game content from user input (URLs or PDFs). Built with React 19, TypeScript, Vite, and powered by Mistral for text generation/OCR, Google Gemini for image + background music generation, and ElevenLabs for SFX/TTS.
...
- `GEMINI_API_KEY` (or `GOOGLE_API_KEY`) — Required for Gemini image + background music generation
- `ELEVENLABS_API_KEY` — Optional, enables SFX + boss TTS generation
...
Text game content is generated via Mistral calls and returned as `RunData`; images and background music are generated with Gemini, while SFX/TTS use ElevenLabs. The app is a single-page client-side React app with no router.
...
- `src/services/audioService.ts` — Audio orchestration with request queue (max 3 concurrent). Generates Gemini background music and ElevenLabs SFX + boss TTS.
